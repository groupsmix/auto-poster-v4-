...
import { ProgressBar, RateCard } from "@/components/ui";

const EMPTY_DASHBOARD: HealthDashboard = {
  api_credits: {
...
function QualityScoreTrend({ trend }: { trend: Array<{ date: string; score: number }> }) {
      ...
      <div className="flex items-end gap-1 h-32">
        {trend.map((point) => (
          <div key={point.date} className="flex-1 flex flex-col items-center gap-1" title={`${point.date}: ${point.score}`}>
            <div className="w-full bg-accent rounded-t" style={{ height: `${(point.score / 10) * 100}%` }} />
...
function TopNichesTable({ niches }: { niches: Array<{ niche: string; products: number; avg_score: number; revenue: number }> }) {
...
function ApiCreditsPanel({ credits }: { credits: HealthDashboard["api_credits"] }) {
  return (
    <div className="rounded-xl border border-card-border bg-card-bg p-5">
      <h3 className="text-sm font-semibold text-foreground mb-4">API Credit Usage (Estimated)</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <ProgressBar label="Workers Requests" value={credits.workers_requests.used} max={credits.workers_requests.limit} color="bg-blue-500" />
        <ProgressBar label="KV Reads" value={credits.kv_reads.used} max={credits.kv_reads.limit} color="bg-purple-500" />
        <ProgressBar label="D1 Reads" value={credits.d1_reads.used} max={credits.d1_reads.limit} color="bg-green-500" />
        <ProgressBar label="R2 Storage (GB)" value={credits.r2_storage_gb.used} max={credits.r2_storage_gb.limit} color="bg-orange-500" />
      </div>
    </div>
  );
}
...
export default function HealthPage() {
  const { data, loading } = useApiQuery(() => api.healthDashboard.get(), EMPTY_DASHBOARD);
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <RateCard label="Workflow Success" rate={data.workflow_success_rate} total={data.workflow_total} failed={data.workflow_failed} color={rateColor(data.workflow_success_rate)} />
            <RateCard label="Publish Success" rate={data.publish_success_rate} total={data.publish_total} failed={data.publish_failed} color={rateColor(data.publish_success_rate)} />
            <div className="rounded-xl border border-card-border bg-card-bg p-5">
          ...
          <ApiCreditsPanel credits={data.api_credits} />
          <QualityScoreTrend trend={data.quality_score_trend} />
          <TopNichesTable niches={data.top_niches} />
        </div>
      )}
    </div>
  );
